...
contract GlobalsAndUtility is ERC20 {
    ...
    */
    ...
    /* ERC20 constants */
    string public constant name = "ILIRA";
    string public constant symbol = "ILIRA";
    uint8 public constant decimals = 8;

    /* Hearts per Satoshi = 10,000 * 1e8 / 1e8 = 1e4 */
    uint256 private constant HEARTS_PER_ILIRA = 10**uint256(decimals); // 1e8
    uint256 private constant ILIRA_PER_BTC = 1e4;
    uint256 private constant SATOSHIS_PER_BTC = 1e8;
    uint256 internal constant HEARTS_PER_SATOSHI =
        (HEARTS_PER_ILIRA / SATOSHIS_PER_BTC) * ILIRA_PER_BTC;
    ...
    /* Seed for WAAS Lobby */
    uint256 internal constant WAAS_LOBBY_SEED_ILIRA = 1e9;
    uint256 internal constant WAAS_LOBBY_SEED_HEARTS =
        WAAS_LOBBY_SEED_ILIRA * HEARTS_PER_ILIRA;
    ...
    uint256 private constant BPB_MAX_ILIRA = 150 * 1e6;
    uint256 internal constant BPB_MAX_HEARTS = BPB_MAX_ILIRA * HEARTS_PER_ILIRA;
    ...
    uint8 internal constant ETH_ADDRESS_ILIRA_LEN = ETH_ADDRESS_BYTE_LEN * 2;

    uint8 internal constant CLAIM_PARAM_HASH_BYTE_LEN = 12;
    uint8 internal constant CLAIM_PARAM_HASH_ILIRA_LEN =
    ...
    bytes internal constant STD_CLAIM_PREFIX_STR = "Claim_ILIRA_to_0x";
    bytes internal constant OLD_CLAIM_PREFIX_STR = "Claim_BitcoinILIRA_to_0x";

    bytes16 internal constant ILIRA_DIGITS = "0123456789abcdef";
    ...
    function dailyDataUpdate(uint256 beforeDay) external {
        ...
        require(g._currentDay > CLAIM_PHASE_START_DAY, "ILIRA: Too early");

        if (beforeDay != 0) {
            require(
                beforeDay <= g._currentDay,
                "ILIRA: beforeDay cannot be in the future"
            );
     ...
     * @return list array of packed values
     */
    ...
    function _stakeLoad(
        ...
        require(
            ...
            "ILIRA: stakeIdParam not in stake"
        );
...
contract StakeableToken is GlobalsAndUtility {
        ...
        external
    {
        ...
        require(
            ...
            "ILIRA: newStakedDays lower than minimum"
        );
    ...
    function stakeGoodAccounting(
        ...
        require(stakeLists[stakerAddr].length != 0, "ILIRA: Empty stake list");
        require(
            stakeIndex < stakeLists[stakerAddr].length,
            "ILIRA: stakeIndex invalid"
        );
        ...
        require(
            ...
            "ILIRA: Stake not fully served"
        );

        /* Stake must still be locked */
        require(st._unlockedDay == 0, "ILIRA: Stake already unlocked");
    ...
    function stakeEnd(uint256 stakeIndex, uint40 stakeIdParam) external {
        ...
        /* require() is more informative than the default assert() */
        require(stakeListRef.length != 0, "ILIRA: Empty stake list");
        require(stakeIndex < stakeListRef.length, "ILIRA: stakeIndex invalid");
        ...
        if (g._currentDay >= st._lockedDay) {
            ...
            /* Deny early-unstake before an auto-stake minimum has been served */
            require(!st._isAutoStake, "ILIRA: Auto-stake still locked");
    ...
    function _stakeStart(
        ...
        require(
            ...
            "ILIRA: newStakedDays higher than maximum"
        );
        ...
        require(
            newStakeShares != 0,
            "ILIRA: newStakedHearts must be at least minimum shareRate"
        );
    ...
    function _stakeStartBonusHearts(
        ...
        /*
            ...
            Bonus percentage scaled 0% to 10% for the first 150M ILIRA of stake.
...
contract UTXOClaimValidation is StakeableToken {
    ...
    function btcAddressIsClaimable(
        ...
        require(
            ...
            "ILIRA: Claim phase has not yet started"
        );
        require(day < CLAIM_PHASE_END_DAY, "ILIRA: Claim phase has ended");
    ...
    function claimMessageMatchesSignature(
        ...
        require(v >= 27 && v <= 30, "ILIRA: v invalid");
    ...
    function pubKeyToBtcAddress(
        ...
        */
        ...
        if (compressed) {
            ...
            require(!(nested && bech32), "ILIRA: claimFlags invalid");
    ...
    function _btcAddressIsValid(
        ...
        /*
            ...
            internal node will never have the zero-fill, as guaranteed by ILIRA's Merkle
            tree construction.
            ...
            ILIRA's Merkle tree construction, which eliminates the only edge-case where
        ...
        if (claimParamHash == 0) {
            return
                abi.encodePacked(
                    ...
                    uint8(prefixStr.length) + ETH_ADDRESS_ILIRA_LEN,
                    prefixStr,
                    addrStr
                );
        }

        bytes memory claimParamHashStr = new bytes(CLAIM_PARAM_HASH_ILIRA_LEN);

        _ILIRAStringFromData(
            claimParamHashStr,
            claimParamHash,
        ...
        return
            abi.encodePacked(
                ...
                uint8(prefixStr.length) +
                    ETH_ADDRESS_ILIRA_LEN +
                    1 +
                    CLAIM_PARAM_HASH_ILIRA_LEN,
                prefixStr,
                addrStr,
                "_",
                claimParamHashStr
    ...
    {
        addrStr = new bytes(ETH_ADDRESS_ILIRA_LEN);
        _ILIRAStringFromData(
            addrStr,
    ...
    function _ILIRAStringFromData(
        bytes memory ILIRAStr,
        bytes32 data,
        uint256 dataLen
    ) private pure {
        uint256 offset = 0;
        ...
        for (uint256 i = 0; i < dataLen; i++) {
            ...
            ILIRAStr[offset++] = ILIRA_DIGITS[b >> 4];
            ILIRAStr[offset++] = ILIRA_DIGITS[b & 0x0f];
        }
    }
...
contract UTXORedeemableToken is UTXOClaimValidation {
     ...
     * @param autoStakeDays Number of days to auto-stake, subject to minimum auto-stake days
     * @param referrerAddr Eth address of referring user (optional; 0x0 for no referrer)
     * @return totalClaimedHearts number of Hearts credited, if successful
     */
    function _satoshisClaim(
        ...
        require(
            ...
            "ILIRA: CHK: _claimedBtcAddrCount"
        );
        ...
        )
    {
        ...
        require(
            ...
            "ILIRA: CHK: _claimedSatoshisTotal"
        );
...
contract TransformableToken is UTXORedeemableToken {
    ...
    function xfLobbyEnter(address referrerAddr) external payable {
        ...
        require(enterDay < CLAIM_PHASE_END_DAY, "ILIRA: Lobbies have ended");

        uint256 rawAmount = msg.value;
        require(rawAmount != 0, "ILIRA: Amount required");
    ...
    function xfLobbyExit(uint256 enterDay, uint256 count) external {
        require(enterDay < _currentDay(), "ILIRA: Round is not complete");
        ...
        if (count != 0) {
            require(
                count <= qRef.tailIndex - headIndex,
                "ILIRA: count invalid"
            );
            endIndex = headIndex + count;
        } else {
            endIndex = qRef.tailIndex;
            require(headIndex < endIndex, "ILIRA: count invalid");
        }
    ...
    function xfLobbyFlush() external {
        require(address(this).balance != 0, "ILIRA: No value");
        ...
        returns (uint256 rawAmount, address referrerAddr)
    {
        ...
        require(entry.rawAmount != 0, "ILIRA: Param invalid");
     ...
     * @return words vector of lobby day numbers
     */
...
contract ILIRA is TransformableToken {
    constructor() {
